...
from constants import PERSISTENT_WORKERS, TrainHyperParams
from io_utils import save_checkpoint, save_json
from my_types import TrainingHistory
...
def create_loader(
    ...
    use_persistent_workers: bool = PERSISTENT_WORKERS if persistent_workers is None else persistent_workers
    return DataLoader(
        dataset,
...
def _train_one_epoch(
    ...
    progress = tqdm(loader, desc=f"Train {epoch_idx + 1}/{total_epochs}", leave=False, dynamic_ncols=True)
...
def _validate_one_epoch(
    ...
    for images, targets in tqdm(loader, desc=f"Val {epoch_idx + 1}/{total_epochs}", leave=False, dynamic_ncols=True):
        try:
            images = images.to(device)
            target_tensor: Tensor = targets.to(device=device, dtype=torch.long)
